...
const ServiceCard = ({service}) => {
    ...
    return (
        <div>
            <div style={{height:'550px'}} className="card card-compact bg-gray-200 shadow-xl p-5">
             <PhotoProvider>
             <PhotoView src={image}>
             <figure><img src={image} alt="animel" /></figure>
             </PhotoView>
             </PhotoProvider>
    ...
    <Link to={`/detailspage/${_id}`} ><button className="btn btn-primary">Details <FaArrowRight className='ml-5 font-bold text-black'/></button></Link>  
    </div>
  </div>
</div>
        </div>
